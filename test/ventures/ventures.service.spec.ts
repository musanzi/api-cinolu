...
import { VenturesService } from '@/modules/ventures/ventures/services/ventures.service';
...
describe('VenturesService', () => {
  ...
  it('toggles publish to approved and emits approved event', async () => {
    ...
    ventureRepository.findOneOrFail.mockResolvedValue({ id: 'v1', is_published: false });
  ...
  it('toggles publish to rejected and emits rejected event', async () => {
    ...
    ventureRepository.findOneOrFail.mockResolvedValue({ id: 'v1', is_published: true });
  ...
  it('finds ventures by user unpaginated', async () => {
    const { service, ventureRepository } = setup();
    ventureRepository.find.mockResolvedValue([{ id: 'v1' }]);
    await expect(service.findByUserUnpaginated('u1')).resolves.toEqual([{ id: 'v1' }]);
  });
  ...
  it('throws on findAll failure', async () => {
    ...
    await expect(service.findAll({} as any)).rejects.toThrow('bad');
  });
  ...
  it('updates venture by slug', async () => {
    ...
    ventureRepository.findOneOrFail.mockResolvedValue({ id: 'v1', name: 'old' });
  ...
  it('sets logo', async () => {
    const { service, ventureRepository } = setup();
    ventureRepository.findOneOrFail.mockResolvedValue({ id: 'v1', logo: null });
  ...
  it('sets cover', async () => {
    const { service, ventureRepository } = setup();
    ventureRepository.findOneOrFail.mockResolvedValue({ id: 'v1', cover: null });
