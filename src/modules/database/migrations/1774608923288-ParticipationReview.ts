...
export class ParticipationReview1774608923288 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`project_participation_review\` ADD CONSTRAINT \`FK_24eda18de407252409a6ea395d1\` FOREIGN KEY (\`participationId\`) REFERENCES \`project_participation\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`
    );
    await queryRunner.query(
      `ALTER TABLE \`project_participation_review\` ADD CONSTRAINT \`FK_19da817d9c4383c2f5ac72a7a3c\` FOREIGN KEY (\`phaseId\`) REFERENCES \`phase\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`
    );
    await queryRunner.query(
      `ALTER TABLE \`project_participation_review\` ADD CONSTRAINT \`FK_fcac3cd91a5ec556adf5808748d\` FOREIGN KEY (\`reviewerId\`) REFERENCES \`user\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`project_participation_review\` DROP FOREIGN KEY \`FK_fcac3cd91a5ec556adf5808748d\``
    );
    await queryRunner.query(
      `ALTER TABLE \`project_participation_review\` DROP FOREIGN KEY \`FK_19da817d9c4383c2f5ac72a7a3c\``
    );
    await queryRunner.query(
      `ALTER TABLE \`project_participation_review\` DROP FOREIGN KEY \`FK_24eda18de407252409a6ea395d1\``
    );
