...
export class ParticipationReview1774603700948 implements MigrationInterface {
    name = 'ParticipationReview1774603700948'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`project_participation\` DROP FOREIGN KEY \`FK_eb97ab6335894371c338c940bc9\``);
        await queryRunner.query(`CREATE TABLE \`project_participation_review\` (\`id\` uuid NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`message\` text NULL, \`score\` int NOT NULL DEFAULT '0', \`participationId\` uuid NULL, \`phaseId\` uuid NULL, UNIQUE INDEX \`IDX_63049ae90e05beae845e5799ae\` (\`participationId\`, \`phaseId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
